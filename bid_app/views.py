...
def signup(request):
    if request.method == 'POST':
        ...
        if not all([f, u, e, p, con, add, d2, reg, i]):
        ...
        try:
            with transaction.atomic():
                ...
                AuctionUser.objects.create(membership=mem,user=user,contact=con,address=add,dob=d2,image=i, user_type=reg)
...
def Add_Product(request):
    ...
    if request.method == "POST":
        ...
        for i in range(int(numberofparts)):
            ...
            parts_name = request.POST.get(f'parts_name-{i}', None)
            parts_image = request.FILES.get(f'parts_image-{i}', None)
...
def make_participants(request, pid):
    ...
    send_bid_notification_email(request.user, product=product.name)
...
def startbidding(request, pid):
    ...
    if participant1.first():
        ...
        if minutes.seconds/60 >= 10 and product.status == "live":
            product.winner = participant.first().user
            product.status = "closed"
            product.save()
            send_bid_notification_email.delay(product.winner, product.name)
